...
export default function NewlyAdded() {
  ...
  // Filter for most recent (Tail end of the array, reversed)
  const vlogs = [...videosData].reverse().slice(0, 12);
  ...
  return (
    <section className="responsive-section" style={{ backgroundColor: 'white' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '0 0.5rem' }}>
        <div>
          <h2 style={{ fontSize: '2.5rem', fontWeight: 900, color: 'var(--text-main)', letterSpacing: '-0.04em' }}>New Videos</h2>
          <p style={{ color: 'var(--text-dim)', fontSize: '1rem', fontWeight: 500 }}>New videos from this week.</p>
        </div>
        <Link href="/videos" style={{ color: 'var(--accent-primary)', fontWeight: 800, fontSize: '0.8rem', letterSpacing: '0.1rem', textTransform: 'uppercase' }}>See what's new →</Link>
      </div>
            ...
            {vlogs.map(v => (
              ...
              >
                 <div style={{ position: 'relative', height: '200px', borderRadius: 'var(--radius-lg)', overflow: 'hidden', boxShadow: 'var(--shadow-md)', backgroundColor: 'var(--bg-deep)' }} className="glass-hover">
                    ...
                    <span style={{ position: 'absolute', top: '0.8rem', left: '0.8rem', background: '#10b981', color: 'white', padding: '4px 8px', borderRadius: '4px', fontSize: '0.65rem', fontWeight: 800 }}>NEW</span>
